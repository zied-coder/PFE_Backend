...
public class SessionControleServiceImplTest {
    ...
    @Test
    void addSessionControle() {
        Contribuable contribuable = new Contribuable();

        when(contribuableRepo.save(contribuable)).thenReturn(contribuable);

        Contribuable addedcontribuable = contribuableService.saveContribuable(contribuable);

        assertEquals(contribuable, addedcontribuable);
        verify(contribuableRepo, times(1)).save(contribuable);
    }
